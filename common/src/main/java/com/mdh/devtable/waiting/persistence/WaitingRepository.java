...
import com.mdh.devtable.waiting.persistence.dto.UserWaitingQueryDto;
...
public interface WaitingRepository extends JpaRepository<Waiting, Long> {
    ...
    @Query("""           
             select new com.mdh.devtable.waiting.persistence.dto.UserWaitingQueryDto(s.id, w.id, s.name, s.shopType, s.region.city, s.region.district, w.waitingNumber, w.waitingPeople.adultCount, w.waitingPeople.childCount)
             from Waiting w
    ...
    @Query("""
            select
                new com.mdh.devtable.waiting.persistence.dto.WaitingDetails
                (
    ...
    @Query("""
            SELECT new com.mdh.devtable.waiting.persistence.dto.WaitingInfoResponseForOwner(w.waitingNumber, u.phoneNumber)
            FROM Waiting w
